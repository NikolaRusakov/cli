 ...
 *
 * The same types and storages are used both for local `code-pushup collect`
 * runs (via the `@code-pushup/collect-plugin` plugin) and for CI runs
 * (e.g. `@code-pushup/azure-devops-ci`).
 */

/**
 * Source where the run was triggered.
 *
 * - `local`  – `code-pushup collect` invoked on a developer machine
 * - `ci`     – run triggered from a CI pipeline
 */
export type RunSource = 'local' | 'ci';
