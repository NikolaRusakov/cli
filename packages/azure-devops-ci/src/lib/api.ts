/* eslint-disable n/no-unsupported-features/node-builtins, max-lines-per-function, @typescript-eslint/no-non-null-assertion, complexity, functional/no-let, functional/no-loop-statements, @nx/enforce-module-boundaries */
...
function isCodePushUpThread(thread: AzureDevOpsThread): boolean {
  ...
  const firstComment = thread.comments[0];
  return (
    firstComment?.content?.includes('generated by @code-pushup/ci') ?? false
  );
}
...
export function parseCollectionUri(uri: string): {
  ...
  const vsMatch = trimmedUri.match(/^https?:\/\/([^.]+)\.visualstudio\.com$/);
  if (vsMatch) {
    return {
